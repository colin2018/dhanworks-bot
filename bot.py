...
# Inline Keyboards
# =========================
# 【修改点】已删除 inline_back_to_main 函数
...
# Content
# =========================
# 【修改点】已不再使用 home_text 显示长文案，改用极简提示
...
def handle_start(message: dict):
    ...
    # 2. 随后展示主菜单
    # 【修改点】删除原有长欢迎文案，改为极简提示，仅用于唤起 Reply Keyboard
...
def handle_callback_query(update: dict):
    ...
    # 【修改点】移除了 "nav:home" 的处理逻辑（因为按钮已删除，但保留代码亦无害）
    if data == "nav:home":
        answer_callback(cq_id, "✅")
    ...
    if data in map_responses:
        answer_callback(cq_id, "✅")
        # 【修改点】移除了 reply_markup=inline_back_to_main()
